...
const contributionDay = contributions.weeks.map((week) => ({
  ...week,
...
<img src="/images/893946.jpg" alt="${user.name}'s avatar" class="w-16 h-16 rounded-full border-2 border-primary" />

<div>
  <h2 class="text-xl font-bold text-primary">${user.name || user.login}</h2>
  <p class="text-sm text-faint">@${user.login}</p>
</div>
...
```
<grid class="grid grid-cols-4" style="grid-auto-rows: auto;">
  <card class="card colspan-4">
  <h2>Daily Contributions</h2>
  <h3>Lorem Ipsum</h3>
  ${resize((width, height) => (Plot.cell(dailyContributions.slice(-365), {
    width,
    height,
    x: (d) => d.date.getUTCDate(), 
    y: (d) => d.date.getUTCMonth(), 
